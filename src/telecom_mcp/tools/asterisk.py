...
def _connectors(ctx: Any, pbx_id: str) -> tuple[Any, AsteriskAMIConnector, AsteriskARIConnector]:
    ...
    if not target.ami or not target.ari:
        raise ToolError(NOT_FOUND, f"Asterisk target missing AMI/ARI configuration: {pbx_id}")
...
def pjsip_show_endpoint(ctx: Any, args: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    pbx_id = _require_pbx_id(args)
    ...
    try:
        ami_response = ami.send_action({"Action": "PJSIPShowEndpoint", "Endpoint": endpoint})
    finally:
        ami.close()
...
def pjsip_show_endpoints(ctx: Any, args: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    pbx_id = _require_pbx_id(args)
    filter_obj = args.get("filter") if isinstance(args.get("filter"), dict) else {}
    ...
    if isinstance(starts_with, str):
        items = [i for i in items if str(i.get("endpoint", i.get("ObjectName", ""))).startswith(starts_with)]
    if isinstance(contains, str):
        items = [i for i in items if contains in str(i.get("endpoint", i.get("ObjectName", "")))]

    return {"type": target.type, "id": target.id}, norm.normalize_pjsip_endpoints(items, limit)


def active_channels(ctx: Any, args: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    pbx_id = _require_pbx_id(args)
    filter_obj = args.get("filter") if isinstance(args.get("filter"), dict) else {}
    limit = args.get("limit", 200)
    if not isinstance(limit, int) or limit < 1:
    ...
    if isinstance(state, str):
        channels = [c for c in channels if str(c.get("state", c.get("ChannelStateDesc", ""))) == state]
    if isinstance(caller, str):
        channels = [c for c in channels if caller in str(c.get("caller", c.get("CallerIDNum", "")))]
    if isinstance(callee, str):
        channels = [c for c in channels if callee in str(c.get("callee", c.get("ConnectedLineNum", "")))]

    return {"type": target.type, "id": target.id}, norm.normalize_active_channels(channels, limit)
